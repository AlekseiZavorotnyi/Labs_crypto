...
class Feistel_network: public IEncryptionRound, public IKeyExpansion, public ISymmetricCipher{
private:
    mass_mass_b roundKeys_;
    bool were_keysSetup = false;

protected:
    virtual mass_b F_function(mass_b& inputBlock, mass_b& roundKey) = 0;
    virtual mass_mass_b key_expansion(mass_b& input_key) = 0;
    ...
    mass_b en_de_crypt(mass_b& block, bool encrypt) {
        ...
        for (auto K_cur : keys){
            mass_b temp = R_prev;
            mass_b res_F = F_function(R_prev, K_cur);
...
public:
    ...
    mass_b encryptRound(mass_b& inputBlock, mass_b& roundKey) override {
        return F_function(inputBlock, roundKey);
    }

    mass_mass_b key_extension(mass_b& input_key) override {
        return key_expansion(input_key);
    }
    ...
    void setupKeys(mass_b& key) override {
        were_keysSetup = true;
        roundKeys_ = key_expansion(key);
    }
};

class DES : public Feistel_network{
public:
    DES() = default;

    explicit DES(ByteOrder byte_order) : byte_order(byte_order) {}

    mass_b F_function(mass_b& inputBlock, mass_b& roundKey) override {
    ...
    mass_mass_b key_expansion(mass_b& input_key) override{
        if (input_key.size() != 8) {
        ...
        for (int i = 1; i <= 16; i++){
            ...
            RES_KEYS[i - 1] = P_block(bitsetToBytes<56>(comb_C_D), PC2, byte_order, StartIndex::ONE);
        }
        return RES_KEYS;
    }
...
class DEAL : public Feistel_network {
public:
    DEAL() : des_adapter_() {}
    explicit DEAL(ByteOrder byte_order) : byte_order(byte_order), des_adapter_() {}

    mass_b F_function(mass_b& inputBlock, mass_b& roundKey) override {
        std::cout << "=== DEAL::F_function ===" << std::endl;
        std::cout << "inputBlock size: " << inputBlock.size() << std::endl;
        std::cout << "roundKey size: " << roundKey.size() << std::endl;

        if (inputBlock.size() != 8) {
            throw std::invalid_argument("DEAL F-function input must be 8 bytes");
        }
