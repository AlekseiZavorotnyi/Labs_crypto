...
void DEALRound::encryptRound(const uint8_t* inputBlock, const uint8_t* roundKey, uint8_t* output) {
    ...
    DESAdapter local_des_adapter_;

    local_des_adapter_.encrypt(inputBlock, roundKey, temp1);
    local_des_adapter_.decrypt(temp1, modifiedKey1, temp2);
    local_des_adapter_.encrypt(temp2, modifiedKey2, output);
}