#include "./DES/DESRound.h"

DESRound::DESRound(ByteOrder order) : byte_order(order) {}

void DESRound::applyE(const uint8_t* inputBlock, uint8_t* output) {
    static const int E[] = {
...
void DESRound::applySboxes(const uint8_t* input48, uint8_t* output32) {
    ...
    for (int sbox = 0; sbox < 8; sbox++) {
        int startBit = 16 + (7 - sbox) * 6;

        int row = ((input64 >> (startBit + 5)) & 1) << 1;
        row |= ((input64 >> (startBit + 0)) & 1);

        int col = (input64 >> (startBit + 1)) & 0xF;
...
void DESRound::applyP(const uint8_t* input32, uint8_t* output32) {
...
void DESRound::encryptRound(const uint8_t* inputBlock, const uint8_t* roundKey, uint8_t* output) {
    uint8_t expanded[6];
    uint8_t xored[6];
    uint8_t sboxed[4];
